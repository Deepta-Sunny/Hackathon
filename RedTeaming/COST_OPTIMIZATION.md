...
Quick next steps
----------------
- Add a small telemetry summary to `api_server.py` that logs model call counts and token estimates per run.  
- Implement a retention policy process (script) to archive/compress old `attack_results/` artifacts.
