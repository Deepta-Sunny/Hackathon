...
class ChatbotWebSocketTarget:
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # Consume the initial "Connected" message if it exists
            try:
                initial_msg = await asyncio.wait_for(self.websocket.recv(), timeout=2.0)
                # print(f"DEBUG: Consumed initial message: {initial_msg}") 
            except asyncio.TimeoutError:
                pass # No initial message, which is fine
            except Exception:
                pass 
